# Security Notes a- Path Traversal / "Zip Slip"-like bypasses
  ...
  - Notes: Anchored semantics interpret input as if rooted under the anchor;
    absolute symlink targets are clamped to the anchor (virtual filesystem semantics).Class Review (soft-canonicalize + anchored)
...
| Symlink semantics: relative vs absolute        | —                              | Both relative and absolute symlinks are clamped to anchor (virtual filesystem semantics) | src/tests/anchored_security/symlink.rs::{relative_symlink_keeps_clamp,absolute_symlink_is_clamped}; src/tests/anchored_symlink_clamping.rs::*                         | src/tests/symlink_dotdot_symlink_first.rs::*; src/tests/symlink_dotdot_resolution_order.rs::*                          |
