...
- **Switched to `junction-verbatim` 1.3.0 for stability and MSRV compatibility**: Replaced DK26 junction fork (GitHub dependency) with the dedicated `junction-verbatim` crate published on crates.io. This provides a frozen, stable version while maintaining Rust 1.70 support. The upstream `junction` v1.4.1 requires Rust 1.71+, which would break our MSRV guarantee.
